...
async function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('create-react-liferay.generate', async function () {
        try {
            // Importando dinamicamente as funções do seu próprio pacote Liferay-React!
            const { processProject } = require('@liferay-react/create-react-liferay/bin/actions.js');
            const { normalizeDirName, formatDisplayName, getJavaVersion } = require('@liferay-react/create-react-liferay/bin/utils.js');

            // 1. Pergunta o Tipo de Módulo
            const typeOptions = [
                { label: '$(symbol-class) Widget React', description: 'Módulo Padrão', value: 'widget' },
                { label: '$(package) Shared Bundle', description: 'Dependências Compartilhadas', value: 'shared' }
            ];
            ...
            if (selectedType === 'shared') {
                selectedTemplate = { name: 'Shared Bundle', dir: 'templates/shared-bundle', hasShared: false };
            ...
            // 3. Fluxo: Widget React
                ...
                const templateOptions = [
                    { label: 'Módulo simples', value: { name: 'Simples', dir: 'templates/modules/src-simple', hasShared: false } },
                    { label: 'Módulo simples com template pré-determinado', value: { name: 'Com Template Pré-determinado', dir: 'templates/modules/src-simple-with-template', hasShared: true } },
                    { label: 'Módulo com Shared (básico)', value: { name: 'Com Shared', dir: 'templates/modules/src-shared', hasShared: true } },
                    { label: 'Módulo com Shared CLI', value: { name: 'Com Shared CLI', dir: 'templates/modules/src-shared-cli', hasShared: true } }
                ];
                
                const selectedTemplateObj = await vscode.window.showQuickPick(templateOptions, { 
                ...
                portletCategory = await vscode.window.showInputBox({ 
                    prompt: 'Qual a categoria do portlet? (ex: category.simple, category.hidden)', 
                ...
                if (selectedTemplate.hasShared) {
                    ...
                    finalSharedBundle = await vscode.window.showInputBox({ 
                        ...
                        placeHolder: 'Ex: boilerplate-shared',
                        validateInput: text => text.trim() ? null : 'O nome do diretório do shared bundle é obrigatório.'
                    });
            ...
            const liferayPath = await vscode.window.showInputBox({ 
                prompt: 'Qual o caminho (relativo ou absoluto) para o bundle do Liferay (liferayDir)?', 
                ...
                validateInput: text => {
                    ...
                    try {
                        ...
                        return (fs.existsSync(resolvedPath) && fs.lstatSync(resolvedPath).isDirectory()) ? null : 'A pasta do Liferay não foi encontrada. Verifique o caminho digitado.';
                    } catch (e) {
            ...
            const templateDirObj = require.resolve('@liferay-react/create-react-liferay/package.json');
            ...
            vscode.window.withProgress({
                ...
                vscode.window.showInformationMessage(`✅ Módulo ${finalProjectName} criado com sucesso!`);
            });
